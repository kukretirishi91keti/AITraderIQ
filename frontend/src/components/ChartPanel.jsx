...
function ChartPanel({ history, selectedSymbol, chartInterval, currency }) {
  ...
  try {
    ...
    const points = history
      .map((h, i) => {
        ...
        const y =
          height -
          padding -
          (((h.close || h.price || 0) - minPrice) / range) * (height - 2 * padding);
        return `${x},${y}`;
      })
      .join(' ');
    ...
    const firstMs = firstTimestamp
      ? new Date(typeof firstTimestamp === 'string' && !firstTimestamp.includes('T')
          ? firstTimestamp.replace(' ', 'T')
          : firstTimestamp).getTime()
      : 0;
    const lastMs = lastTimestamp
      ? new Date(typeof lastTimestamp === 'string' && !lastTimestamp.includes('T')
    ...
    const displayStartLabel =
      startLabel ||
      (chartInterval.includes('m') || chartInterval.includes('h')
        ? new Date(Date.now() - history.length * 60000).toLocaleTimeString('en-US', {
            hour: '2-digit',
            minute: '2-digit',
          })
        : new Date(Date.now() - history.length * 86400000).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
          }));
