      ...
      <div className="p-4 overflow-y-auto max-h-[60vh]">
        {portfolio.length === 0 ? (
          <p className="text-gray-400 text-center py-8">
            Your portfolio is empty. Add stocks using the &quot;+ Portfolio&quot; button.
          </p>
        ) : (
          <table className="w-full">
            ...
            <tbody>
              {portfolio.map((pos, i) => {
                const currentPrice =
                  quote?.symbol === pos.symbol
                    ? quote.price
                    : pos.avgPrice * (1 + (Math.random() - 0.5) * 0.1);
                ...
                return (
                  <tr key={i} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                    ...
                    <td
                      className={`py-3 text-right font-medium ${pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                    >
                      {pnl >= 0 ? '+' : ''}
